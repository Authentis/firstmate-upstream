...
assert_grep "$REMOTE_ROOT/bin/fm-inbox-ack.sh' '$PARENT_ROUTE_INBOX' NNN.msg" "$REMOTE_HOME/data/charter.md" "remote charter did not render the inbox acknowledgement host-local"
assert_no_grep "$ROOT/bin/fm-inbox-ack.sh" "$REMOTE_HOME/data/charter.md" "remote charter retained the local inbox acknowledgement helper path"
