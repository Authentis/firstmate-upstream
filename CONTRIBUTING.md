...
A suite that drives the real `treehouse` binary gives its fixtures a private pool root, as `herdr_private_treehouse_root` in `tests/herdr-test-safety.sh` does, because `bin/fm-test-run.sh` hands each suite a stand-in for the user Treehouse root and fails any suite that creates a pool there.
