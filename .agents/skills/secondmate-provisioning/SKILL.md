...
Remote branch retirement needs no propagation: a secondmate home runs the same tracked `bin/fm-pr-merge.sh`, `bin/fm-teardown.sh`, and ship-brief rule as the primary, so its merged head branches and proven-landed `preserve/`, `archive/`, and `work/*-baseline` refs retire, and its lanes keep safety refs off origin, under the rules those scripts' headers own.
