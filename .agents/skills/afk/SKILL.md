...
3. **Announce, then read back after entry.**
   ...
   Say plainly which sentence, if any, you could not act on while away (a red merge, a discard, anything on the never-set, or local-only landing unless the read-back shows standing landing on); it waits for their return.
...
- On Pi, main is parked and the supervision branch handles every safe actionable wake under main's standing authority, through the same guarded scripts main would use: any pull request green at its live head may merge (which one the words meant is the branch's reading), queued work whose blockers cleared - already queued, or filed by the branch because the words explicitly call for it - dispatches within the spend cap, and a decision is answered with the captain's own pre-stated answer or under `ask-user-authority`.
  Anything else holds for the return, a red merge never proceeds while away, local-only landing waits for the captain unless the home carries the standing `config/afk-land-green` setting (then green, in-scope work - pull requests and local-only branches alike - lands through the guarded merge scripts without the words naming it; `bin/fm-branch-prompt.sh` "Postures" owns what in scope means), and only a wake the branch declines (including a broken branch or unsafe scan) or a watcher failure wakes main (`docs/pi-supervision-branch.md` "Postures").
