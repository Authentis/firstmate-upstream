...
#       line naming that exact id.
#     - `deferred_owner` / `deferred_id`: from the latest disposition event
#       when its disposition is "deferred", otherwise null.
...
fm_nm_findings_fold() {  # <data-dir> <task-id>
  ...
  _fm_nm_findings_valid_events "$ledger" | jq -s '
    ...
    | ($seens | group_by(.finding.id) | map({
        id: .[0].finding.id,
        finding: .[0].finding,
        first_seen: {round: .[0].round, step: .[0].step},
        last_seen: {round: (.[-1].round), step: (.[-1].step)}
      })) as $folded_seen
    | ($disps | group_by(.finding_id) | map(.[-1])) as $latest_disp
    | $folded_seen | map(
        . as $f
        | ($latest_disp | map(select(.finding_id == $f.id)) | .[0]) as $d
        | $f + {
