...
- Standing landing: when the read-back carries a `standing landing: on` line (the captain's `config/afk-land-green` setting), green, in-scope work lands without the words naming it: a pull request green at its live head through `bin/fm-pr-merge.sh` as above, or a local-only branch whose worker reported it ready with its named head on the ship branch through `bin/fm-merge-local.sh`, which then accepts you and keeps every one of its own gates; then the ordinary cleanup follows.
  In scope means the change does what its instructions asked, no decision or captain hold on the task is open, and nothing about it is destructive, irreversible, or security-sensitive; anything short of that holds for the return.
  This setting is the one standing exception to never-by-analogy below; log each such landing like any action under the words, naming the standing landing setting as the sentence acted on.
- Without that line `bin/fm-merge-local.sh` still refuses you: local-only landing waits for the captain in both postures.
