...
{claude,cursor} 3. `supervision-host: cycle boundary ...` means the host ended its park at its bound: run `bin/fm-wake-drain.sh --ack-if-routine`, handle whatever it presents, run its printed acknowledgement unless it printed `ROUTINE:` (an empty queue prints `--ack-through 0`), and end the turn; the next park starts at that turn end.
{opencode,omp} 3. `supervision-host: cycle boundary ...` means the host ended its park at its bound and the next park has already started: run `bin/fm-wake-drain.sh --ack-if-routine`, handle whatever it presents, and run its printed acknowledgement unless it printed `ROUTINE:` (an empty queue prints `--ack-through 0`).
{grok} 3. `supervision-host: cycle boundary ...` means the host ended its park at its bound: run `bin/fm-wake-drain.sh --ack-if-routine`, handle whatever it presents, run its printed acknowledgement unless it printed `ROUTINE:` (an empty queue prints `--ack-through 0`), and re-arm the same background host call.
