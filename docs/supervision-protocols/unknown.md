...
Ordinary wake: drain with `bin/fm-wake-drain.sh --ack-if-routine` (a `ROUTINE:` line means it already acknowledged a wake with nothing to handle), handle all emitted wakes, reconcile open decisions and unread status lines, and run the exact `--ack-through` command printed as `WAKE_ACK_REQUIRED`, then repeat that verified wait while supervision is still required.
