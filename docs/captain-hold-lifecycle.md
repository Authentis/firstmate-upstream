...
The pending-close record teardown already stages before destructive cleanup carries that intent as a `mode=retain` line, so a cleanup interrupted after removing its task record replays the retention at the next session start through the same record, validator, and lock as an ordinary close and never closes the row, while one interrupted before that keeps both records for a teardown rerun; if the captain answers before replay, `answer` validates that record and copies any supported retained pull request or report into the row before closing it, after which replay retires the record.
...
The exact commands and their summarized outputs are recorded in the shipping PR's evidence; run the four suites above plus `tests/fm-send-resolve-key.test.sh`, `tests/fm-bearings-board.test.sh`, the three `tests/fm-procevent*.test.sh` runner suites, and `bin/fm-lint.sh` to refresh this record, and `FM_BEARINGS_LAVISH_LIVE=1 tests/fm-bearings-board-lavish-live-e2e.test.sh` after a lavish-axi upgrade.
