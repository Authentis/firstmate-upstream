...
A `refs/for/<branch>` push creates no upstream tracking ref, so nothing ever reads `[gone]`; the prune retires such a ship branch only through its content leg, once no live task record names it and its content is on `origin/<default>`.
