...
`exit` stops an agent and preserves every record, worktree, and change.
Removing a worktree, closing an endpoint, or discarding work stays with [`bin/fm-teardown.sh`](../bin/fm-teardown.sh), which owns the landed-work test; exit's Herdr pane hand-off calls that owner rather than closing anything itself, so a closed agent does not leave an unused pane open, while relaunch's internal stop keeps the endpoint it reuses.
